...
class Helper:
    ...
    def __init__(self, parse_arguments_fn, proc_id):
        logging.basicConfig(stream=sys.stdout, level=logging.INFO)

        # set_args
        parser = argparse.ArgumentParser()
        args = parse_arguments_fn(parser)
        args.exp_name = self.get_exp_name(*[getattr(args, attr) for attr in args.exp_name_attr]) + f"-proc:{proc_id}"
    ...
    def log(self, k: str, v: str):
        logging.info(f"{setproctitle.getproctitle()}->{k}: {v}")
    ...
    def save_checkpoint(self, model, name):
        ...
        logging.info(f"{setproctitle.getproctitle()}->Save checkpoint to: {path}")
        return

    def load_checkpoint(self, model, path):
        model.load_state_dict(torch.load(path, map_location=self.args.device))
        logging.info(f"{setproctitle.getproctitle()}->Load checkpoint from: {path}")
        return
    ...
    def save_checkpoint_if_best(self, model, name, scores):
        ...
        if save:
            logging.info(f"{setproctitle.getproctitle()}->New best {self.args.monitor} score ({score:.4f})")
            self.best_score = score
            self.save_checkpoint(model, name)
        return

    def save_predictions(self, predictions):
        logging.info(f"{setproctitle.getproctitle()}->Save predictions to: {self.prediction_saved_filename}")
    ...
    def get_source_file_list(suffix=None):
        ...
        if suffix is None:
            suffix = ["py", "ipynb"]
        return nested_list_reduce([glob.glob(f"{project_path}/**/*.{suffix}", recursive=True) for suffix in suffix])
